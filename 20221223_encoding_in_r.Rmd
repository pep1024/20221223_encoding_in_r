...
result <- rawToChar(s_raw[s_raw >= as.raw("0x20")])
result
```
<!-- 
...
seq_ex <- 92:96
seq_hex1 <- sprintf("%x", seq_ex)
print(seq_hex1)
seq_ex2 <- strtoi(seq_hex1, base = 16L) 
print(seq_ex2)

## Turn sequence into hexadecimal numbers sequence
seq_hex3 <- as.hexmode(seq_hex1)
seq_hex4 <- paste0("0x", seq_hex1)

## convert it to characters
rawToChar(as.raw(seq_hex3))
rawToChar(as.raw(seq_hex4))
rawToChar(as.raw(seq_ex))
```
...
chars2 <-  sprintf("%#04x", 861:866)
chars <- "\\#H0065"

grid::grid.newpage()
grid::grid.text(chars, x = 1:6/7, gp = grid::gpar(fontfamily="HersheySans"))
```
```{r}
grid::grid.text("hello", x = 1/4)
```
